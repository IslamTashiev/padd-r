...
const mobileWallets = [
  ...
  },
  {
    ...
    icon: "🛡️",
    deepLink:
      "https://link.trustwallet.com/open_url?coin_id=60&url=https://padd-r.io",
  },
  {
  ...
  const tryConnectorsSequentially = async () => {
    ...
    for (const connector of ordered) {
      ...
      try {
        ...
        setErrorMsg(
          `Ошибка при подключении через ${connector.name}: ${
            err?.message || err
          }`
        );
      }
    ...
    setErrorMsg(
      "Не удалось подключиться ни через один из доступных кошельков."
    );
  };
